...
public class SlidingDoor : MonoBehaviour, IInteractableObserver
{
    ...
    private HashSet<InteractionObject> interactablesSet = new();
    ...
    private void Awake()
    {
        interactablesSet = interactablesList
            .Select(obj => obj.GetComponent<InteractionObject>())
            .Where(interactionObject => interactionObject != null)
            .ToHashSet();

        closePosition = transform.localPosition;
        openPosition = openDirection switch
        {
    ...
    {
        foreach (InteractionObject interactable in interactablesSet)
        {
    ...
    private IEnumerator ActivateDoor(Vector3 endPosition, float activationTime)
    {
        ...
        float adjustedActivationTime = activationTime * (remainingDistance / Vector3.Distance(closePosition, openPosition));
